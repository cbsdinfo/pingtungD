...
export const PerTaskHistory = (props) => {
    ...
    useEffect(() => {
        const historyUnlisten = history.listen((location, action) => {
            ...
            globalContextService.remove("PerTaskHistoryPage", "firstUseAPIgetTodayTask");
        });

        return () => {
            globalContextService.remove("PerTaskHistoryPage")
            historyUnlisten();
        }
    }, [])
    //#endregion
    ...
    const getTodayTask = useCallback(async (useAPI = false, newsCategoryId = "", releaseDate = fmt(moment(), "YYYY-MM")) => {
        ...
        if (isUndefined(globalContextService.get("PerTaskHistoryPage", "firstUseAPIgetTodayTask")) || useAPI) {
            //#endregion

            //#region 取得所有最新消息類別 API
            await fetch(`${APIUrl}OrderOfCaseUsers/GetCaseOrderCourseByDriver?driverid=${getParseItemLocalStorage("DriverID")}`, //categorys/load?page=1&limit=20&TypeId=SYS_DRIVER_LICENSE
                {
                    headers: {
                ...
                .finally(() => {
                    ...
                    globalContextService.set("PerTaskHistoryPage", "firstUseAPIgetTodayTask", false);
                    //#endregion
                });
            //#endregion
